...
export async function handleEvolutionWebhook(req: Request) {
  ...
  try {
    ...
    instanceName = typeof rawInstance === "string" ? rawInstance : undefined;
    const data = body?.data ?? body?.payload ?? body;

    if (!normalizedEvent || !instanceName) return NextResponse.json({ status: "ok" });

    dbInstance = await WhatsAppInstance.findOne({ instanceName });
    if (!dbInstance) {
    ...
    switch (normalizedEvent) {
      ...
      case "CONNECTION_UPDATE":
        const status = data.statusReason;
        if (status === "open") {
          dbInstance.status = "connected";
          dbInstance.qrCode = undefined;
        ...
        for (const msg of messages) {
          ...
          if (messageType === "audio" && ENV.BLOB_READ_WRITE_TOKEN) {
            ...
            try {
              ...
              const finalMime = parsed.mimetype || (mediaMeta as any)?.mimetype || "audio/ogg";
    ...
    try {
      if (dbInstance) {
        dbInstance.lastWebhookAt = new Date();
        dbInstance.lastWebhookError = String(error?.message || error);
        await dbInstance.save();
      } else if (instanceName) {
        await WhatsAppInstance.updateOne(
          { instanceName },
          { $set: { lastWebhookAt: new Date(), lastWebhookError: String(error?.message || error) } }
        );
      }
    } catch {
      // ignore
